...
import { macroSeriesList } from "../macro.js";

const PIPELINE_MACRO_URL = "https://pipeline.test/macro";
...
describe("fred-macro-daily job adapter", () => {
  it("dry-runs: with no pipeline URL it runs no fetches and reports no work", async () => {
    ...
    try {
      const job = fredMacroDailyJob(env({ PIPELINE_MACRO_URL: undefined }));
      expect(job.scope).toBe("batch");
      expect(job.id).toBe("fred-macro-daily");
  ...
  it("publishes every series, shares one run_id, and reports no failures", async () => {
    ...
    try {
      const job = fredMacroDailyJob(env());
  ...
  it("records a per-series failure without aborting the rest", async () => {
    ...
    vi.stubGlobal("fetch", async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = String(input);
      if (url.includes("stlouisfed.org")) {
        if (url.includes("series_id=CPIAUCSL") && !url.includes("units=pc1")) {
    ...
    try {
      const job = fredMacroDailyJob(env({ MACRO_CONCURRENCY: 1 }));
      const run = await job.run(
        UNIVERSE,
        env({
          PIPELINE_MACRO_URL,
      ...
      expect(run.failures[0].symbol).toBe("CPIAUCSL");
      expect(run.runId).toBe("run-1");
