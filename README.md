...
| `GET /api/news?symbol=&limit=` | Upcoming-ish per-ticker news headlines (Worker → Tavily news search; `{title, link, published, snippet}`, cached in-isolate + D1 ~10 min). Feeds the AI Copilot's `get_news` tool — the narrative half of "why is vol high". |
