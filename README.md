...
renders only the published panels. Interactive chat (and the Account menu)
also pick a **reply voice** — canned audiences `desk` (working trader),
`fund` (hedge-fund / PM), or `learner` (new to trading), plus an optional
240-character note. Voice only: same Copilot tools and desk as everyone else.
Bot `system_prompt_extra` is capped at 1000 characters so timeline personas
cannot dump unbounded context. The Worker owns the schema context,
