...
| `GET /api/experiments/kalshi-parlays` | Live Kalshi parlay study (public, design `kalshi-parlays-v4`). Listed `KXFEDCOMBO` cells vs independence of `KXFEDDECISION` × `KXFOMCDISSENTCOUNT` legs; homemade index/crypto parlays scored with a Gaussian copula using lake OHLC return correlation; sports parlays from `options.kalshi_markets` (MVE combos + selected legs). Combo independence / implied ρ require a two-sided book inside (0, 1) — RFQ 0/0/0 is not a listed quote. Crypto 15m CROSSCATEGORY stacks are scored separately from NFL/sports props. Isolate cache ~10 min on a scored snapshot, ~90 s after a Kalshi 429/empty miss. |
