...
| `POST /api/me/avatar` | Upload a custom avatar (`multipart/form-data` field `avatar`, or raw image body). JPEG/PNG/WebP/SVG, ≤5 MB. Stored in Cloudflare Images; D1 keeps the image id. SVG is screened for script/handler payloads (Images also sanitizes). Requires a claimed handle. Returns `{ok, name, display_name, avatar_url}`. |
