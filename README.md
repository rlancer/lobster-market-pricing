...
| `GET /api/share/{id}/el5` | Public EL5 rewrite of that post (`{share_id, el5, cache_hit, computed_at, model}`). First viewer generates via OpenRouter and stores the Markdown in D1 keyed by `share_id` + source hash; later viewers are cache hits. `?force=1` regenerates. 20 generations / 10 min / IP. |
...
| `POST /api/admin/email/test` | Admin session — Cloudflare Email Service smoke test. Sends from `noreply@lobster.mp` to the signed-in admin's email. Returns `{ ok, to, message_id }`. |
