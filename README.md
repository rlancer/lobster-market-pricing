...
| `GET /api/schwab/pnl` | Realized trading PnL time series from TRADE history (`range=MTD\|YTD\|1M\|3M\|6M\|1Y`, optional `account`). FIFO lot matching; synthesizes zero-cash option covers when Schwab posts assignment stock delivery without an option close; cumulative curve plus closing-fill rows (fees, realized) and `DIVIDEND_OR_INTEREST` distributions for the Portfolio Performance pane. Not an equity curve (excludes deposits/withdrawals and open MTM). |
