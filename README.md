...
| `GET /api/experiments/kalshi-parlays` | Live Kalshi parlay study (public, design `kalshi-parlays-v6`). Listed `KXFEDCOMBO` cells vs independence of `KXFEDDECISION` × `KXFOMCDISSENTCOUNT` legs; homemade index/crypto parlays scored with a Gaussian copula using lake OHLC return correlation; sports RFQ-auction parlays from `options.kalshi_markets`. Same-game stacks report corr room (Fréchet high − p×q). Auction prints require `yes_last` in (0, 1) and volume > 0. Crypto 15m/daily CROSSCATEGORY stacks are scored separately. Isolate cache ~10 min on a scored snapshot, ~90 s after a Kalshi 429/empty miss. |
