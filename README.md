...
| `GET /api/experiments/kalshi-parlays` | Live Kalshi parlay study (public, design `kalshi-parlays-v7`). Listed `KXFEDCOMBO` cells vs independence of `KXFEDDECISION` × `KXFOMCDISSENTCOUNT` legs; homemade index/crypto parlays scored with a Gaussian copula using lake OHLC return correlation; sports RFQ-auction parlays from `options.kalshi_markets`. Same-game stacks report corr room (Fréchet high − p×q). Combo mids include solicited RFQ two-ways (`source=kalshi_rfq`, cancelled without a fill) and auction prints (`yes_last` in (0, 1) and volume > 0). Crypto 15m/daily CROSSCATEGORY stacks are scored separately. Isolate cache ~10 min on a scored snapshot, ~90 s after a Kalshi 429/empty miss. |
...
| `GET /api/admin/kalshi-parlay` | Admin — Kalshi same-game parlay RFQ executor monitor. Proxies loader `GET /jobs/kalshi-parlay-executor`, `GET /jobs/kalshi-markets-hourly`, and `GET /loop/status`. Stable JSON with `execute` / `live` / `idle_reason`, RFQ counts, universe mix, and decision rows. Powers `/admin/kalshi-parlay`. Never calls Kalshi from the browser. |
| `POST /api/admin/kalshi-parlay/trigger` | Admin — force an async dry-run pass (`POST …/jobs/kalshi-parlay-executor/trigger?force=1&async=1` with server-side `LOADER_TOKEN`). **503** if the token is missing. Cannot turn `KALSHI_PARLAY_LIVE` on. |
