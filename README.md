...
| `GET /api/experiments/kalshi-parlays` | Live Kalshi parlay study (public, ~10 min isolate cache). Listed `KXFEDCOMBO` cells vs independence of `KXFEDDECISION` × `KXFOMCDISSENTCOUNT` legs; homemade index/crypto parlays scored with a Gaussian copula using lake OHLC return correlation; census of the public MVE combo CLOB. |
