...
| `GET /api/experiments/kalshi-parlays` | Live Kalshi parlay study (public). Listed `KXFEDCOMBO` cells vs independence of `KXFEDDECISION` × `KXFOMCDISSENTCOUNT` legs; homemade index/crypto parlays scored with a Gaussian copula using lake OHLC return correlation; sports parlays from `options.kalshi_markets` (`theme=sports`, open MVE combos + selected legs). Isolate cache ~10 min on a scored snapshot, ~90 s after a Kalshi 429/empty miss. |
