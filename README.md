...
`GET /api/schwab/pnl` (realized trading PnL series: MTD / YTD / 1M / 3M / 6M / 1Y; optional `as_of=YYYY-MM-DD` to end the window on that ET date).
...
with a combined Portfolio pane (performance chart + open positions; search
a ticker to scope both) and Trade history. User-facing copy is
...
| `GET /api/experiments/kalshi-parlays` | Live Kalshi parlay study (public, design `kalshi-parlays-v9`). Listed `KXFEDCOMBO` cells vs independence of `KXFEDDECISION` × `KXFOMCDISSENTCOUNT` legs; homemade index/crypto parlays scored with a Gaussian copula using lake OHLC return correlation; sports RFQ-auction parlays from `options.kalshi_markets`. Same-game stacks report corr room (Fréchet high − p×q). Combo mids include solicited RFQ two-ways (`source=kalshi_rfq`, cancelled without a fill) and auction prints (`yes_last` in (0, 1) and volume > 0). Settlement 0/1 is a tagged `source=kalshi_settlement` row. Executor fills land as `source=kalshi_parlay_fill`. The snapshot `backtest` grades those live fills (actual side vs YES-at-the-same-price) and the live same-game RFQ filter (BUY YES at the ask, 10 contracts). Crypto 15m/daily CROSSCATEGORY stacks are scored separately. Isolate cache ~10 min on a scored snapshot, ~90 s after a Kalshi 429/empty miss. |
| `GET /api/experiments/kalshi-parlay-books` | Settlement-graded bakeoff of sports parlay books (public, design `kalshi-parlay-books-v1`): corr-room YES vs underdog / payoff YES vs always-YES/NO on executor fills and RFQ two-ways. Same payload as `GET /api/experiments/kalshi-parlay-payoffs` (YES-price buckets vs hit rate / breakeven). Isolate cache ~10 min. |
...
| `GET /api/schwab/pnl` | Realized trading PnL time series from TRADE history (`range=MTD\|YTD\|1M\|3M\|6M\|1Y`, optional `account` + `symbol` + `as_of=YYYY-MM-DD` to spoof the ET end date). FIFO lot matching on the America/New_York calendar; synthesizes option covers when Schwab posts assignment stock delivery without an option close (nearest expiry when several shorts share a strike); point sleeves for equity / option / fees / dividends; window `trades[]` plus closing-fill rows and `DIVIDEND_OR_INTEREST` distributions for the Schwab Portfolio pane. Not an account-balance curve (excludes deposits/withdrawals). Ticker-scoped UI adds live open mark and daily paths from Schwab Market Data on the connected user token (`ohlc[]`; option legs are Black–Scholes on those stock closes, IV from the fill). Assignment moves delivered-stock intrinsic loss onto the short option so settlement does not shock net P&L. When the extended cost-basis lookback fails, `lookback_truncated: true` and the response uses the chart window only. User help: `/docs/schwab-pnl`. Matching rules: this README, Schwab Performance matching. |
...
Session card sits below that (mobile and desktop): live index tape (indexes plus
the front two VX monthals), the next
