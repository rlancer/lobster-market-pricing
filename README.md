...
| `GET /api/experiments/kalshi-parlays` | Live Kalshi parlay study (public, design `kalshi-parlays-v5`). Listed `KXFEDCOMBO` cells vs independence of `KXFEDDECISION` × `KXFOMCDISSENTCOUNT` legs; homemade index/crypto parlays scored with a Gaussian copula using lake OHLC return correlation; sports parlays from `options.kalshi_markets` (MVE combos + selected legs). Combos are RFQ auctions (HVM): empty 0/0/0 is the resting book; independence / implied ρ use a two-sided book or an auction print (`yes_last` in (0, 1)). Crypto 15m/daily CROSSCATEGORY stacks are scored separately from NFL/sports props. Isolate cache ~10 min on a scored snapshot, ~90 s after a Kalshi 429/empty miss. |
