...
| `GET /api/experiments/kalshi-parlays` | Live Kalshi parlay study (public, design `kalshi-parlays-v8`). Listed `KXFEDCOMBO` cells vs independence of `KXFEDDECISION` × `KXFOMCDISSENTCOUNT` legs; homemade index/crypto parlays scored with a Gaussian copula using lake OHLC return correlation; sports RFQ-auction parlays from `options.kalshi_markets`. Same-game stacks report corr room (Fréchet high − p×q). Combo mids include solicited RFQ two-ways (`source=kalshi_rfq`, cancelled without a fill) and auction prints (`yes_last` in (0, 1) and volume > 0). Settlement 0/1 is a tagged `source=kalshi_settlement` row. The snapshot `backtest` grades the live same-game RFQ filter (BUY YES at the ask, 10 contracts) against those outcomes, and reports the BUY NO P&L that 2026-09-14 production fills actually took. Crypto 15m/daily CROSSCATEGORY stacks are scored separately. Isolate cache ~10 min on a scored snapshot, ~90 s after a Kalshi 429/empty miss. |
