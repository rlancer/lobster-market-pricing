...
export function pageMetaForUrl(pathname: string, search = ''): PageMeta {
  ...
  if (path === '/portfolio') {
    return {
      ...
      description:
        'Public lobster suggested-trade performance, an optional paper book, and live Schwab trade history when connected.',
      path: '/portfolio',
    };
  }
  ...
  if (path === '/experiments/text-vs-image' || path === '/notebooks/text-vs-image') {
    return {
      ...
      description:
        'Compare AI-style text, the same text as rasterized images, labeled chart images, and textless charts with a markdown color key on a dense synthetic equity panel (80 names × 1y).',
