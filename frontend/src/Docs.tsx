...
const JOBS = [
  ...
  ['kalshi-markets-hourly', 'Hourly', 'Curated Kalshi event contracts (Fed/CPI/indexes/crypto/oil) plus sports parlays (MVE combos + selected legs, ~30d candles, settlement 0/1) → options.kalshi_markets.'],
];
...
const ENDPOINTS: { method: string; path: string; desc: ReactNode }[] = [
  ...
  { method: 'GET', path: '/api/tables', desc: 'Lake tables with columns/types, row counts, distinct key counts, and sample rows (D1-cached; ?force=1 recomputes live)' },
  ...
  { method: 'GET', path: '/api/admin/kalshi-parlay', desc: 'Admin — Kalshi same-game parlay RFQ executor last_pass (execute/live/idle_reason, universe counts, considered legs + skip reasons, would_accept decisions). Proxies the loader; never calls Kalshi from the browser.' },
  ...
  { method: 'GET', path: '/api/experiments/kalshi-parlays', desc: 'Public live snapshot: Kalshi Fed combo mids vs independence of rate/dissent legs, homemade index/crypto parlays with lake return correlation, and sports RFQ-auction parlays from options.kalshi_markets (design kalshi-parlays-v9: solicited RFQ two-ways, same-game corr room vs independence, settlement-graded live fills + filter backtest, volume-backed auction prints, sports vs crypto split)' },
  { method: 'GET', path: '/api/experiments/kalshi-parlay-books', desc: 'Public live snapshot: settlement-graded bakeoff of Kalshi sports parlay books (corr-room YES vs underdog / payoff YES vs always-YES/NO) on executor fills + RFQ two-ways. Design kalshi-parlay-books-v1. Same payload as /api/experiments/kalshi-parlay-payoffs (price-bucket calibration).' },
...
const SURFACES = [
  {
    route: '/',
    title: 'Floor',
    body: 'The Floor — the home feed of chats people chose to share publicly. When you are signed in with Schwab connected, a Your book card sits at the top of the feed: equity, day PnL with DTD percent, open PnL, and the largest open lots with per-lot DTD returns (live `GET /api/schwab/status` then `GET /api/schwab/portfolio`). Open book goes to `/portfolio?book=schwab`; a position opens Research on that root; Ask about the book starts Chat with the Schwab book attached. Visitors without a connected book never see it — no connect CTA on the feed. A Session card below that shows the live index tape (SPY/QQQ/IWM/DIA + front two VX monthals), the next high-impact print (CPI/Jobs/FOMC), and the latest @nowlobster desk takeaway — the reason to check the site during the session, including on mobile where the companion rail is hidden. The Session card reads a precomputed Worker snapshot (`GET /api/timeline/session`, D1-backed, warmed by the 5-minute cron) so the homepage does not wait on lake or calendar queries. Ask about the tape opens Chat with a prompt grounded in that snapshot. Each post is a card (discrete conversation you can share, reply to, or unpublish — not a divider row) and shows the full conversation in place (tall threads expand without leaving the feed) using the same transcript body as live chat — Thinking, every tool call, every SQL query, charts, desk viewpoints, and query results (hydrated when you expand). Session frames open in a Sources strip with Open in Data, same as /chat. The title opens the unlisted /share/<id> page, and a share control beside the title (and on each assistant reply) offers copy link or system Share via… — reply links deep-link with `#m-N`. A reply icon beside EL5 opens a modal with the full conversation and a follow-up composer that forks the transcript into your own chat (Google sign-in + public handle required) so the continue is attributed to you when shared. On desktop a companion column shows trending tags, breaking news, and an index tape (hidden on small viewports). Posts are attributed to a handle; a handle’s profile and public chats live at /u/<handle>, which reuses the same companion column.',
  },
  {
    route: '/u/<handle>',
    title: 'Profile',
  ...
  },
  {
    route: '/experiments',
    title: 'Experiments',
    body: 'Public studies. Kalshi parlay mispricing compares listed Fed combos to independence of the rate and dissent legs, homemade index/crypto parlays scored with lake return correlation, and sports RFQ-auction parlays from options.kalshi_markets (MVE combos plus selected legs). Empty combo books are the RFQ venue; the hourly ingest solicits a capped same-game RFQ two-way (source=kalshi_rfq) and cancels without filling. Design v9 backtests that live filter against settlement 0/1 (source=kalshi_settlement) and reports YES-at-ask P&L versus the BUY NO fills from 2026-09-14. Two companion notebooks bake off alternate books (corr-room YES vs underdog / payoff YES) and bucket combo YES prices versus empirical hit rate (/experiments/kalshi-parlay-books, /experiments/kalshi-parlay-payoffs, design kalshi-parlay-books-v1). Crypto 15m/daily CROSSCATEGORY stacks are scored separately from NFL props. Text vs image compares encodings on a synthetic panel. Analyst desk vs sessions freezes an as-of snapshot and grades solo / production desk role-play / isolated specialist sessions against held-out 5- and 20-session direction. Trading firm pipeline reuses that tape and grades TradingAgents-style stages (reports, bull/bear, later risk committee). Published LLM runs load from the Worker so visitors do not spend OpenRouter credits.',
  },
  {
    route: '/data',
    title: 'Data',
