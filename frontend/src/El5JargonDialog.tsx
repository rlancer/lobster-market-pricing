...
/** Leaf model id for meta — drop provider prefix and dated build tags. */
function shortEl5Model(model: string): string {
...
export function El5PostButton({
  ...
  const modelLabel = translation?.model?.trim()
    ? shortEl5Model(translation.model.trim())
    : null;
