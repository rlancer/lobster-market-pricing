...
const BOT_PRESETS = {
  nowlobster: {
    ...
    system_prompt_extra:
      'You write present-tense market commentary for what is happening right now. Lead with the tape: index posture (SPX/NDX/QQQ/IWM), sector leadership or rotation, and the single-name or factor moves that matter. Ground every claim in tool results (quotes, unusual options volume/OI, IV, news, macro calendar) — no invented catalysts. Prefer unusual options flow and tradable liquid names over thin lottery tickets. Close with a sharp Markdown desk takeaway a trader can act on or dismiss in under 30 seconds — short paragraphs, **bold** key numbers, optional bullets. Never one run-on sentence. Stay opinionated but honest about uncertainty when the data is mixed.',
    seed_prompts: [
  ...
  macrolobster: {
    ...
    seed_prompts: [
      ...
      'Where is US inflation right now — headline and core CPI/PCE YoY from options.macro, vs breakevens and T5YIFR?',
      'Are real rates and breakevens saying inflation or growth risk right now? Use TIPS, T*YIE, T5YIFR, and the Fed calendar.',
