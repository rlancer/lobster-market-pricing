...
export const FEEDS: CatalogItem[] = [
  ...
  },
  {
    ...
    description:
      'Hourly snapshots of investing-relevant Kalshi markets into options.kalshi_markets — Fed funds / FOMC decisions, CPI, GDP, S&P/Russell/Dow levels, BTC/ETH ranges, WTI — plus every open two-leg sports parlay (theme=sports, empty 0/0 CLOB included) and the legs those combos select, including ~30 days of daily candles (volume-capped), tagged settlement 0/1 rows (source=kalshi_settlement; two-leg / RFQ / fill tickers still settle when they miss that candle cap), solicited RFQ two-ways (source=kalshi_rfq), and executor fills (source=kalshi_parlay_fill). Not the full Kalshi catalog and not n>2 as the parlay universe. Combos must name at least one sports leg — pure crypto 15m/daily CROSSCATEGORY stacks are not sports parlays. Combo rows encode collection + selected tickers in category as mve|{collection}|{yes|no}:{LEG}@{EVENT},…. Investing rows carry optional related_symbol (SPY, TLT, BTC-USD, …) for joins. Powers /research/{ticker} event-market rails via GET /api/research/{ticker}/kalshi (related_symbol only — sports 0/0 rows do not change latest-wins Fed/CPI/index research), the /experiments/kalshi-parlays notebook (including the live-fill backtest), the /experiments/kalshi-parlay-books and /experiments/kalshi-parlay-payoffs bakeoff, Chat event-vol joins, and Kalshi trade suggestions (investing series only). Local DuckDB in notebooks/.cache is a session memo, not this table.',
    provider: 'Kalshi',
...
export const TABLE_META: Record<string, Pick<CatalogItem, 'summary' | 'description' | 'feeds' | 'tools'>> = {
  ...
  kalshi_markets: {
    ...
    description:
      'Investing-relevant Kalshi markets (Fed/CPI/GDP/indexes/crypto/oil) plus every open two-leg sports parlay (theme=sports, category mve|…, empty 0/0 CLOB included): series_ticker, market_ticker, title, theme, yes_bid/yes_ask/yes_last (0–1 dollars), volume/OI, close_time, related_symbol. Hourly snapshots; latest-wins on market_ticker for research (related_symbol only — sports rows do not change Fed/CPI/index research), full RFQ/settlement/fill history for the parlay experiment. Join related_symbol to options.ohlc / option_contracts for event-vol context. Sports rows have related_symbol=null and are for the parlay experiment — Chat suggest_trades stays on investing series. Combo 0/0/0 books are the RFQ auction resting venue, not listed CLOB quotes. Same-game combo two-ways may be solicited RFQ quotes (`source=kalshi_rfq`); those RFQs are cancelled without accepting. Settled books also carry `source=kalshi_settlement` 0/1 rows (not mixed into candles). Executor accepts land as `source=kalshi_parlay_fill`. Notebooks/.cache DuckDB is a session memo, not this table. Powers /research/{ticker} Event markets (GET /api/research/{ticker}/kalshi) and Kalshi legs in suggest_trades.',
    feeds: ['kalshi'],
    tools: ['run_query', 'suggest_trades'],
  },
  futures_settlements: {
