...
export const FEEDS: CatalogItem[] = [
  ...
  },
  {
    ...
    description:
      'Hourly snapshots of investing-relevant Kalshi markets into options.kalshi_markets — Fed funds / FOMC decisions, CPI, GDP, S&P/Russell/Dow levels, BTC/ETH ranges, WTI — plus sports parlay combos (theme=sports) and the legs those combos select, including ~30 days of daily candles, tagged settlement 0/1 rows (source=kalshi_settlement), and executor fills (source=kalshi_parlay_fill). Not the full Kalshi catalog. Combos must name at least one sports leg — pure crypto 15m/daily CROSSCATEGORY stacks are not sports parlays. Combo rows encode collection + selected tickers in category as mve|{collection}|{yes|no}:{LEG}@{EVENT},…. Investing rows carry optional related_symbol (SPY, TLT, BTC-USD, …) for joins. Powers /research/{ticker} event-market rails via GET /api/research/{ticker}/kalshi (related_symbol only), the /experiments/kalshi-parlays notebook (including the live-fill backtest), Chat event-vol joins, and Kalshi trade suggestions (investing series only).',
    provider: 'Kalshi',
