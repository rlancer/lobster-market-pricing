/**
 * Run the firm-pipeline matrix in-process (OpenRouter from this job), then
 * publish via the Worker admin API.
 *
 * A full firm cell is 10 seats across 4 waves — too many DeepSeek calls for
 * one Worker HTTP request. GitHub Actions has a 3h budget; the Worker does not.
 *
...
function expectedSessions(approachId: string): number {
  if (approachId === "solo") return 1;
  if (approachId === "reports_then_trader") return 5;
  if (approachId === "bull_bear_debate") return 7;
  return 10;
}
...
async function main() {
  const design = await api("/api/experiments/firm-pipeline/design") as {
    design_id: string;
    as_of_rules: string;
    model?: string;
  ...
  const fillMissing = (reason: string) => {
    ...
    for (const key of executionOrder) {
      ...
      cells.push({
        ...
        session_count: expectedSessions(repId!),
      });
    }
  };
    ...
    for (const experimentCase of design.cases) {
      ...
      for (let attempt = 1; attempt <= PROBE_ATTEMPTS; attempt += 1) {
        try {
          const run = await withTimeout(
            runFirmPipelineApproach(approach.id as FirmApproachId, local, complete),
            CELL_TIMEOUT_MS,
            key,
          );
      ...
      if (lastError) {
        ...
        cells.push({
          ...
          model: modelId,
          attempts: PROBE_ATTEMPTS,
          session_count: expectedSessions(approach.id),
        });
