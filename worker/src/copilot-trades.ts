...
export function tradesSuggestBlock(): string {
  return [
    ...
    "- Kalshi legs: instrument=kalshi + market_ticker from options.kalshi_markets (curated Fed/CPI/index/crypto/oil + mega-cap company_event series) + optional contract_side yes|no (default yes). buy/sell is the Kalshi contract side. Prefer two-sided yes_bid/yes_ask with demonstrated volume. Trade ticker may be the related_symbol (SPY, TLT, META, BTC-USD) or the series_ticker (KXFED, KXMETAANTITRUST).",
