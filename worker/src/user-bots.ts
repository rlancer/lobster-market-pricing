...
export function userBotSystemAddon(opts: {
  ...
  if (source === "paper") {
    lines.push(
      "The owner attached their paper book. MUST call get_paper_portfolio (or get_portfolio with source=paper) before recommending anything. Do not call get_portfolio with source=schwab. Do not invent positions, fills, or balances.",
    );
    ...
    const accountClause = accountIds.length === 1
      ? ` and pass account_id="${accountIds[0]}"`
      : accountIds.length > 1
        ? ` scoped to accounts ${accountIds.map((id) => `"${id}"`).join(", ")}`
        : "";
    lines.push(
      `The owner attached ${label}. MUST call get_portfolio with source="schwab" before recommending anything${accountClause}`
    ...
    const schwabClause = accountIds.length === 1
      ? ` get_portfolio with source="schwab" and account_id="${accountIds[0]}"`
      : accountIds.length > 1
        ? ` get_portfolio with source="schwab" scoped to accounts ${accountIds.map((id) => `"${id}"`).join(", ")}`
        : ` get_portfolio with source="schwab"`;
    lines.push(
    ...
    lines.push(
      "No portfolio is attached. Do not call get_paper_portfolio or get_portfolio unless the owner asks about a book in this prompt.",
    );
  }
  if (source !== "none") {
    lines.push(
