 ...
 */
import { DESK_MARKDOWN_SHAPE, deskAnalystBlock, privateBriefingBlock, type DeskViewpointId } from "./chat-desk";
...
export function systemPrompt(schema: string, botOrOpts?: BotPromptProfile | null | SystemPromptOptions): string {
  ...
  const privateDirect = Boolean(bot && bot.audience === "private" && !bot.publish_to_timeline);
  const lines = [
    ...
    schema,
    "",
    privateDirect ? privateBriefingBlock() : deskAnalystBlock(deskSpecialists),
    "",
    ...(privateDirect ? [] : [tradesSuggestBlock(), ""]),
    "Rules:",
    ...
    "- To answer a market-data question, ALWAYS gather evidence first: get_portfolio / get_paper_portfolio when a book is attached, get_market_tape for overview / what's-going-on / hourly-tape asks, otherwise a read-only query via run_query. Never return only SQL. Dummy probes (SELECT 1, or SELECT 1 FROM options.securities) are rejected — they are not evidence.",
    ...
    "- Curated Kalshi event odds live in options.kalshi_markets (hourly): series_ticker (KXFED, KXCPI, KXINX, KXBTC, …), market_ticker, theme, yes_bid/yes_ask/yes_last (0–1 dollars), volume/OI, close_time, related_symbol. Latest-wins on market_ticker. Use for market-implied probs on Fed/CPI/index weeks — not sports or politics. Sports parlay combos (theme=sports, category mve|…) exist for the parlay experiment only; never suggest sports bets and never treat those rows as investing ideas.",
    ...
    "- Every run_query MUST SELECT real columns FROM at least one options.* lake table (or a CTE that does). Bare probes like SELECT 1, SELECT 'test' AS t, or SELECT 1 AS dummy FROM options.securities are rejected before they hit the lake.",
