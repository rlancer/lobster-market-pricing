...
export function systemPrompt(schema: string, botOrOpts?: BotPromptProfile | null | SystemPromptOptions): string {
  ...
  const lines = [
    ...
    "- Curated Kalshi event odds live in options.kalshi_markets (hourly): series_ticker (KXFED, KXCPI, KXINX, KXBTC, KXMETAANTITRUST, …), market_ticker, theme (rates|inflation|growth|equity_index|crypto|commodity|company_event), yes_bid/yes_ask/yes_last (0–1 dollars), volume/OI, close_time, related_symbol. Latest-wins on market_ticker. Use for market-implied probs on Fed/CPI/index weeks and — when related_symbol matches the name — company litigation / antitrust / KPI / CEO catalysts in fundamental analysis. Sports/entertainment markets are intentionally absent; broad political horse-races without a related_symbol are out of scope.",
    "- When research_ticker returns Related Kalshi event markets, the fundamental specialist MUST weigh those YES odds (e.g. Meta FTC appeal, KPI prints) alongside filings/news — do not ignore them or invent contracts not in the brief / lake.",
    ...
    "- When suggesting a trade or analyzing a specific ticker, MUST call research_ticker first. It lake-normalizes the symbol, links this chat to that security, and returns price/volume technicals, lake fundamentals, earnings, news, and related Kalshi event markets when curated. Ground every specialist take in that brief plus follow-up SQL.",
