...
export function systemPrompt(schema: string, botOrOpts?: BotPromptProfile | null | SystemPromptOptions): string {
  ...
  const lines = [
    ...
    "- Identify holdings before concentration or single-name claims. Broker books include asset kind + description on each line (COLLECTIVE_INVESTMENT / etf means a fund, not a stock). If a ticker is unlabeled or you are unsure whether it is a single issuer vs an ETF/fund/index, MUST call lookup_symbols — the lake does not cover every ticker, but Yahoo search still returns quoteType + name. Single-name concentration is one issuer (AAPL, SBNY). A diversified index/equal-weight/broad-market ETF is sleeve/beta size — never label it \"single-holding\" or recommend trimming it as if it were a stock. Sector/thematic ETFs are factor concentration; commodity products are commodity concentration.",
