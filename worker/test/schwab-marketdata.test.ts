...
import { CHAT_TOOL_INPUT_SCHEMAS } from "../src/chat-contract.ts";
...
test("get_schwab_quotes schema accepts symbols only — never a user id", () => {
  assert.deepEqual(
    CHAT_TOOL_INPUT_SCHEMAS.get_schwab_quotes.parse({ symbols: ["AAPL", "MSFT"] }),
    { symbols: ["AAPL", "MSFT"] },
  );
  assert.throws(() =>
    CHAT_TOOL_INPUT_SCHEMAS.get_schwab_quotes.parse({
      symbols: ["AAPL"],
